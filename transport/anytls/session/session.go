...
func NewClientSession(conn net.Conn, _padding *atomic.Value) *Session {
	s := &Session{
		conn:        conn,
		isClient:    true,
		sendPadding: true,
...
func NewServerSession(conn net.Conn, onNewStream func(stream *Stream), _padding *atomic.Value) *Session {
	s := &Session{
		conn:        conn,
...
func (s *Session) Run() {
	...
	settings := util.StringMap{
		"v":           "2",
		"client":      "mihomo/1.1",
		"padding-md5": s.padding.Load().(*padding.PaddingFactory).Md5,
	}
			...
			case cmdSettings:
				if hdr.Length() > 0 {
					...
					if !s.isClient {
						...
						paddingF := s.padding.Load().(*padding.PaddingFactory)
...
func (s *Session) writeDataFrame(sid uint32, data []byte) (int, error) {
	...
	buffer.Grow(dataLen + headerOverHeadSize)
	buffer.WriteByte(cmdPSH)
	binary.Write(buffer, binary.BigEndian, sid)
	binary.Write(buffer, binary.BigEndian, uint16(dataLen))
	buffer.Write(data)
...
func (s *Session) writeControlFrame(frame frame) (int, error) {
	...
	binary.Write(buffer, binary.BigEndian, frame.sid)
	binary.Write(buffer, binary.BigEndian, uint16(dataLen))
